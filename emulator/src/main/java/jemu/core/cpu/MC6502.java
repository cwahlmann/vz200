...
public class MC6502 extends Processor {
	private static final Logger log = LoggerFactory.getLogger(MC6502.class);

	// =============================================================
	// Timings for instructions. This is standard MC6502 T-States.
	// =============================================================

	protected static final byte[] CYCLES = { 7, 6, 0, 0, 0, 3, 5, 5, 3, 2, 2, 0, 0, 4, 6, 0, // 00 .. 0F
	...
	public final void illegal(int opcode) {
		log.warn("Illegal Opcode: " + Util.hex((byte) opcode) + " at " + Util.hex((short) (PC - 1)));
	}
	...
	protected final void step(int opcode) {
		...
		case 0x0a:
			...
			break; // ASL A
		case 0x0d:
			setNZ(A |= memory.readByte(fetchWord()));
			break; // ORA abs
		case 0x0e:
			asl(fetchWord());
			break; // ASL abs
		...
		case 0x11:
			...
			break; // ORA (ind),Y
		case 0x15:
			setNZ(A |= memory.readByte((fetch() + X) & 0xff));
			break; // ORA zp,X
		case 0x16:
			asl((fetch() + X) & 0xff);
			break; // ASL zp,X
		...
		case 0x5d:
			...
			break; // EOR abs,X
		case 0x5e:
			lsr((fetchWord() + X) & 0xffff);
			break; // LSR abs,X
		case 0x60:
			PC = ((pop() | pop() << 8) + 1) & 0xffff;
			break; // RTS
		case 0x61:
			adc(memory.readByte(indx()));
			break; // ADC (ind,X)
		case 0x65:
			adc(memory.readByte(fetch()));
			break; // ADC zp
		case 0x66:
			ror(fetch());
			break; // ROR zp
		...
		case 0x70:
			...
			break; // BVS
		case 0x71:
			adc(memory.readByte(indyrd()));
			break; // ADC (ind),Y
		case 0x75:
			adc(memory.readByte((fetch() + X) & 0xff));
			break; // ADC zp,X
		case 0x76:
			ror((fetch() + X) & 0xff);
			break; // ROR zp,X
		case 0x78:
			...
			break; // SEI
		case 0x79:
			adc(memory.readByte(absrd(Y)));
			break; // ADC abs,Y
		case 0x7d:
			adc(memory.readByte(absrd(X)));
			break; // ADC abs,X
		case 0x7e:
			ror((fetchWord() + X) & 0xffff);
			break; // ROR abs,X
		case 0x80:
			branch();
			break; // BRA
		...
		case 0x91:
			...
			break; // STA (ind),Y
		case 0x94:
			memory.writeByte((fetch() + X) & 0xff, Y);
			break; // STY zp,X
		case 0x95:
			memory.writeByte((fetch() + X) & 0xff, A);
			break; // STA zp,X
		case 0x96:
			memory.writeByte((fetch() + Y) & 0xff, X);
			break; // STX zp,Y
		case 0x98:
			setNZ(A = Y);
			break; // TYA
		case 0x99:
			memory.writeByte((fetchWord() + Y) & 0xffff, A);
			break; // STA abs,Y
		case 0x9a:
			S = X;
			break; // TXS
		case 0x9c:
			memory.writeByte(fetchWord(), 0);
			break; // STZ abs {Undoc}
		case 0x9d:
			memory.writeByte((fetchWord() + X) & 0xffff, A);
			break; // STA abs,X
		case 0x9e:
			memory.writeByte((fetchWord() + X) & 0xffff, A & X);
			break; // STAX abs,X
		case 0xa0:
			setNZ(Y = fetch());
			break; // LDY #
		...
		case 0xa2:
			...
			break; // LDX #
		case 0xa4:
			setNZ(Y = memory.readByte(fetch()));
			break; // LDY zp
		case 0xa5:
			setNZ(A = memory.readByte(fetch()));
			break; // LDA zp
		case 0xa6:
			setNZ(X = memory.readByte(fetch()));
			break; // LDX zp
		case 0xa8:
			setNZ(Y = A);
			break; // TAY
		...
		case 0xaa:
			...
			break; // TAX
		case 0xac:
			setNZ(Y = memory.readByte(fetchWord()));
			break; // LDY abs
		case 0xad:
			setNZ(A = memory.readByte(fetchWord()));
			break; // LDA abs
		case 0xae:
			setNZ(X = memory.readByte(fetchWord()));
			break; // LDX abs
		case 0xb0:
			if ((P & FC) != 0)
				branch();
		...
		case 0xb1:
			...
			break; // LDA (ind),Y
		case 0xb4:
			setNZ(Y = memory.readByte((fetch() + X) & 0xff));
			break; // LDY zp,X
		case 0xb5:
			setNZ(A = memory.readByte((fetch() + X) & 0xff));
			break; // LDA zp,X
		case 0xb6:
			setNZ(X = memory.readByte((fetch() + Y) & 0xff));
			break; // LDX zp,Y
		case 0xb8:
			P &= ~FV;
			break; // CLV
		...
		case 0xba:
			...
			break; // TSX
		case 0xbc:
			setNZ(Y = memory.readByte(absrd(X)));
			break; // LDY abs,X
		case 0xbd:
			setNZ(A = memory.readByte(absrd(X)));
			break; // LDA abs,X
		case 0xbe:
			setNZ(X = memory.readByte(absrd(Y)));
			break; // LDX abs,Y
		case 0xc0:
			cp(Y, fetch());
			break; // CPY #
		case 0xc1:
			cp(A, memory.readByte(indx()));
			break; // CMP (ind,X)
		case 0xc4:
			cp(Y, memory.readByte(fetch()));
			break; // CPY zp
		case 0xc5:
			cp(A, memory.readByte(fetch()));
			break; // CMP zp
		case 0xc6:
			dec(fetch());
			break; // DEC zp
		...
		case 0xca:
			...
			break; // DEX
		case 0xcc:
			cp(Y, memory.readByte(fetchWord()));
			break; // CPY abs
		case 0xcd:
			cp(A, memory.readByte(fetchWord()));
			break; // CMP abs
		case 0xce:
			dec(fetchWord());
			break; // DEC abs
		case 0xd0:
			...
			break; // BNE
		case 0xd1:
			cp(A, memory.readByte(indyrd()));
			break; // CMP (ind),Y
		case 0xd5:
			cp(A, memory.readByte((fetch() + X) & 0xff));
			break; // CMP zp,X
		case 0xd6:
			dec((fetch() + X) & 0xff);
			break; // DEC zp,X
		case 0xd8:
			...
			break; // CLD
		case 0xd9:
			cp(A, memory.readByte(absrd(Y)));
			break; // CMP abs,Y
		case 0xdd:
			cp(A, memory.readByte(absrd(X)));
			break; // CMP abs,X
		case 0xde:
			dec((fetchWord() + X) & 0xffff);
			break; // DEC abs,X
		case 0xe0:
			cp(X, fetch());
			break; // CPX #
		case 0xe1:
			sbc(memory.readByte(indx()));
			break; // SBC (ind,X)
		case 0xe4:
			cp(X, memory.readByte(fetch()));
			break; // CPX zp
		case 0xe5:
			sbc(memory.readByte(fetch()));
			break; // SBC zp
		case 0xe6:
			inc(fetch());
			break; // INC zp
		...
		case 0xea:
			break; // NOP
		case 0xec:
			cp(X, memory.readByte(fetchWord()));
			break; // CPX abs
		case 0xed:
			sbc(memory.readByte(fetchWord()));
			break; // SBC abs
		case 0xee:
			inc(fetchWord());
			break; // INC abs
		case 0xf0:
			...
			break; // BEQ
		case 0xf1:
			sbc(memory.readByte(indyrd()));
			break; // SBC (ind),Y
		case 0xf5:
			sbc(memory.readByte((fetch() + X) & 0xff));
			break; // SBC zp,X
		case 0xf6:
			inc((fetch() + X) & 0xff);
			break; // INC zp,X
		case 0xf8:
			...
			break; // SED
		case 0xf9:
			sbc(memory.readByte(absrd(Y)));
			break; // SBC abs,Y
		case 0xfd:
			sbc(memory.readByte(absrd(X)));
			break; // SBC abs,X
		case 0xfe:
			inc((fetchWord() + X) & 0xffff);
			break; // INC abs,X
	...
	protected final void adc(int val) {
		...
		if ((P & FD) != 0) {
			...
			A = (cy = A + val + cy) & 0xff;
			P = (P & ~(FN | FZ | FV | FC)) | (A == 0 ? FZ : 0) | ((A & 0x80) == 0 ? 0 : FN) | (cy >= 0x100 ? FC : 0)
					| (result < -128 || result > 127 ? FV : 0);
		}
	}
	...
	protected final void sbc(int val) {
		...
		if ((P & FD) != 0) {
			...
			P = (P & ~(FN | FV)) | FC | (A & FN) | ((A & 0x1F0) > 0x17f || (A & 0x1f0) < 0x80 ? FV : 0);
			if ((A & 0xff00) == 0) {
				A = (A - 0x60) & 0xff;
				P &= ~FC;
			}
			...
			int result = (byte) A - (byte) val - cy;
			A = (cy = A - val - cy) & 0xff;
			P = P & ~(FN | FZ | FV | FC) | (A == 0 ? FZ : 0) | ((A & 0x80) == 0 ? 0 : FN) | (cy < 0 ? 0 : FC)
					| (result < -128 || result > 127 ? FV : 0);
		}
	}
	...
	protected final void cp(int reg, int val) {
		P = (P & ~(FN | FZ | FC)) | (reg == val ? FZ : 0) | (reg < val ? 0 : FC) | (((reg - val) & 0x80) == 0 ? 0 : FN);
	}
	...
	public String getState() {
		return "A=" + Util.hex((byte) A) + ", X=" + Util.hex((byte) X) + ", Y=" + Util.hex((byte) Y) + ", S="
				+ Util.hex((byte) S) + ", PC=" + Util.hex((short) PC);
	}
