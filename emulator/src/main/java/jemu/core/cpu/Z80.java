...
public class Z80 extends Processor {
	...
	protected static final byte[] Z80_TIME_PRE = { 4, 10, 7, 6, 4, 4, 7, 4, 4, 11, 7, 6, 4, 4, 7, 4, // 00 .. 0F
	...
	public static final byte[] Z80_TIME_PRE_ED = { 4, 4, 11, 16, 4, 10, 4, 5, 4, 4, 11, 16, 4, 10, 4, 5, // 40 .. 4F
	...
	protected void setTimes(byte[] pre, byte[] post, byte[] preCB, byte[] postCB, byte[] preED, byte[] postED,
			byte[] extra) {
	...
	protected int executeDDFD(int ixiy, int opcode) {
		...
		case 0xb6:
		case 0xbe:
			indexDDFD(ixiy, opcode, CYCLES_EXTRA_IDXNORM);
			break;

		case 0x36:
			indexDDFD(ixiy, opcode, CYCLES_EXTRA_IDXLDIN);
			break;

		case 0xcb:
			indexDDFD(ixiy, opcode, CYCLES_EXTRA_IDXCB);
			break;
...
//    cycle(timeExtra[CYCLES_EXTRA_NMI]);
	...
	protected int getqq(int index) {
		return index == AF ? reg[index] | (reg[index + 1] << 8) : reg[index + 1] | (reg[index] << 8);
	}
	...
	public String getState() {
		String result = "AF :" + Util.hex((short) getqq(AF)) + " HL :" + Util.hex((short) getqq(HL)) + " DE :"
				+ Util.hex((short) getqq(DE)) + " BC :" + Util.hex((short) getqq(BC)) + " IX :" + Util.hex((short) IX)
				+ " IY :" + Util.hex((short) IY) + "\n" + "AF':" + Util.hex((byte) reg[A1]) + Util.hex((byte) reg[F1])
				+ " HL':" + Util.hex((short) getqq(H1)) + " DE':" + Util.hex((short) getqq(D1)) + " BC':"
				+ Util.hex((short) getqq(B1)) + " Cycles: " + Util.hex((int) cycles);
		return result;
	}

	public String[] getRegisterNames() {
		return new String[] { "AF", "BC", "DE", "HL", "AF'", "BC'", "DE'", "HL'", "PC", "SP", "IX", "IY", "I", "R" };
	}
	...
	public int getRegisterValue(int index) {
		...
		case 6:
		case 7:
			result = getqq((index - 1) * 2);
			break;

		case 4:
			result = reg[F1] | (reg[A1] << 8);
			break;
