...
public interface Constants {
	...
	public enum RegisterToken {
		B("^B$", 0), C("^C$", 1), D("^D$", 2), E("^E$", 3), H("^B$", 4), IX_H("^IXh$", 4), IY_H("^IYh$", 4), L("^B$",
	...
	public enum NumToken {
		// @formatter:off
		hex("0x[0-9a-fA-F]{2,4}", s -> Integer.valueOf(s.substring(2), 16)), dez("(\\+|\\-){0,1}[0-9]+",
				s -> Integer.valueOf(s, 10)), okt("0o[0-7]+", s -> Integer.valueOf(s.substring(2), 8)), bin(
						"0b[0-1]{8,16}", s -> Integer.valueOf(s.substring(2), 2)), IXY_O("I[XY].*[+-]{1}.*[0-9]+",
								s -> Integer.valueOf(s.substring(2))), CHR("'.'", s -> (int) s.charAt(1)), CHR2("'..'",
										s -> (int) s.charAt(1) + ((int) s.charAt(2) << 8));
		// @formatter:on
